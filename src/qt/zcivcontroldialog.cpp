...
std::list<std::string> ZReeveControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZReeveControlDialog::listMints;

ZReeveControlDialog::ZReeveControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZReeveControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZReeveControlDialog::~ZReeveControlDialog()
{
    delete ui;
}

void ZReeveControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZReeveControlDialog::updateList()
{
...
void ZReeveControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZReeveControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZReeve_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZReeveControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZReeveControlDialog::GetSelectedMints()
{
...
void ZReeveControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
