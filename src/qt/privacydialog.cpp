                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zREEVE ought to be enough for anybody." - Bill Gates, 2017
    ui->zREEVEpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzREEVESyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzREEVE->setEnabled(false);
        ui->pushButtonMintzREEVE->setToolTip(tr("zREEVE is currently disabled due to maintenance."));

        ui->pushButtonSpendzREEVE->setEnabled(false);
        ui->pushButtonSpendzREEVE->setToolTip(tr("zREEVE is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zREEVEpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzREEVE_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zREEVE is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zREEVE...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zREEVE in ") +
...
void PrivacyDialog::on_pushButtonSpendzREEVE_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zREEVE is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zREEVE
        sendzREEVE();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zREEVE
    sendzREEVE();
}

void PrivacyDialog::on_pushButtonZReeveControl_clicked()
{
    ZReeveControlDialog* zReeveControl = new ZReeveControlDialog(this);
    zReeveControl->setModel(walletModel);
    zReeveControl->exec();
}

void PrivacyDialog::setZReeveControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzReeveSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzREEVE()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Reeve Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zREEVEpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zREEVE is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " REEVE </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zREEVE</b>";
    ...
    // use mints from zReeve selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZReeveControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZReeveControlDialog::GetSelectedMints();
    }
    ...
    ZReeveControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zReeve Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Reeve, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zReeve Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zREEVE </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zREEVE "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zREEVE "));
    ui->labelzREEVEAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzREEVESyncStatus->setVisible(fShow);
}
