...
#if defined(NDEBUG)
#error "Reeve cannot be compiled without assertions."
#endif
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Reeve
...
{
    RenameThread("reeve-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZREEVEMinted()
{
...
void RecalculateZREEVESpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zREEVE supply
...
bool RecalculateREEVESupply(int nHeightStart)
{
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert REEVE to zREEVE
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // Reeve: We use certain sporks during IBD, so check to see if they are
