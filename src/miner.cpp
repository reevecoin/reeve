...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("ReeveMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("ReeveMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("ReeveMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("reeve-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running ReeveMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
